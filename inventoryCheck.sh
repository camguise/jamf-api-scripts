...
for COMPUTER_ID in `cat $COMPUTERS_IDFILE`; do \
   /usr/bin/curl --silent --max-time 30 --request GET --write-out '%{http_code}' --header 'Accept: application/json' --header 'Content-Type: application/json' --url https://devins01.jamfcloud.com/JSSResource/computers/id/$COMPUTER_ID  -u"$JAMF_CREDS" | jq .computer.general.asset_tag | sed -e's/"//g' >> $ASSET_TAGFILE
done;
