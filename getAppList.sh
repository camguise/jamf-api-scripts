...
echo '"App Name","iTunes URL","Total VPP Licenses","Bundle ID","Distribution Method","Version"'
...
for i in ${appIDs[@]}; do
	...
	version=$(echo "${appXML}" | grep -e "<version>" | /usr/bin/perl -lne 'BEGIN{undef $/} while (/<version>(.*?)<\/version>/sg){print $1}' | awk '{ print "\""$0"\""}')
	
	echo "\"${appName}\",${appURL},${appLicenses},${bundleID},${distributionMethod},${version}"
done
